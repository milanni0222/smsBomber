...
read -p 'Message To? use quotation marks ex "6173833897@vtext.com"  : ' uservar2             #Target sms email

read -p 'Enter your email and password like so milanni0222@gmail.com:PASSWORD  : ' uservar3       #email+password
...
for n in {1..200}; do curl --url "smtps://smtp.gmail.com:465" --ssl-reqd --mail-from $uservar3 --mail-rcpt $uservar2 --upload-file mail.txt --user $uservar3 --insecure; done

#to use other email services simply change "smtps://smtp.gmail.com:465" to match SMTP settings

#for n in {1..200}; 50 = how many times to send, value can be changed
